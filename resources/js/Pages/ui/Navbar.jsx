...
const Navbar = ({ sidebarWidth = 70, theme = "light", onThemeChange }) => {
  ...
  return (
    <>
      ...
      <nav
        className={`flex items-center justify-between px-3 sm:px-6 py-2 border-b min-h-[56px] transition-all duration-300 fixed top-0 right-0 z-30 ${theme === "dark"
          ? "bg-darkBg border-neutral-700"
      ...
      >
        <div className="flex flex-1 justify-end items-center gap-3 sm:gap-4 lg:gap-5 min-w-0 overflow-x-hidden">

          {/* Search Icon with modal trigger */}
          <span
            className="block text-xl text-neutral-500 cursor-pointer hover:text-neutral-700 transition-colors"
          ...
          <span className="hidden sm:flex items-center gap-1 cursor-pointer hover:bg-neutral-100 rounded px-2 py-1 transition-colors">
            ...
            <span className={`font-medium text-secondaryText text-sm`}>EN</span>
          </span>


          <span
            className="hidden sm:block text-xl text-gray-500 cursor-pointer hover:text-gray-700 transition-colors"
          ...
          <span className="hidden md:block relative text-xl text-gray-500 cursor-pointer hover:text-gray-700 transition-colors" onClick={handleFullscreenToggle}>
            <img className="h-6 w-6 lg:h-7 lg:w-7" src="https://api.iconify.design/ic:baseline-fullscreen.svg?color=gray" alt="Fullscreen" />
          </span>


          <span className="flex   items-center sm:gap-3   sm:ml-4 flex-shrink-0">
            <img
            ...
            <span className="hidden lg:flex flex-col items-start">
              <span className={`font-semibold text-sm ${theme === "dark" ? "text-primaryText" : "text-gray-900"}`}>
                Json Taylor
              </span>
              <span className={`text-xs text-secondaryText -mt-0.5 ${theme === "dark" ? "text-secondaryText" : ""}`}>Web Designer</span>
            </span>

            <img
              src="https://api.iconify.design/ic:outline-settings.svg?color=gray"
              alt="Settings"
              className="block ml-2 lg:ml-3    animate-spin-slow w-5 h-5 lg:w-5 lg:h-5 cursor-pointer hover:text-gray-700 transition-colors"
            />
          </span>
        </div>
         {showModal && (
        ...
        >
          <div
            className="bg-white rounded shadow p-6 w-[30%] min-w-[200px] mt-12 min-h-[100px]" onClick={e => e.stopPropagation()}
          >
            <span className="block text-lg font-semibold mb-2">Search</span>
            <span className="text-gray-600 text-sm">This is a simple search modal.</span>
          </div>
        </div>
      )}  
      </nav>
